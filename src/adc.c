...
int adc_sample(const struct device *adc_dev)
{
  ...
  const struct adc_sequence_options options =
  {
      ...
      .callback = NULL,
      .user_data = NULL,
      .extra_samplings = SAMPLES_IN_BUFFER-1
  };

  // set adc conversion sequence
  const struct adc_sequence sequence =
  {
  	.options     = &options,                // no extra samples and callback
	.channels    = channel_mask,        // bit mask of channels to read
	.buffer      = payload.sample_buff,         // where to put samples read
	.buffer_size = sizeof(payload.sample_buff),
  ...
  for (int i=0; i < ELEM_BUFF_SAMPLES; i++)
  {

        s32_t milliv = payload.sample_buff[i];
    
        // get millivolt
        adc_raw_to_millivolts(adc_ref_internal(adc_dev), ADC_GAIN, ADC_RESOLUTION, &milliv);
        
        payload.sample_buff[i] = (s16_t)milliv;
         
  }

  return ELEM_BUFF_SAMPLES;
}

s16_t *adc_get_millivolts(void)
{
  return payload.sample_buff;
}
